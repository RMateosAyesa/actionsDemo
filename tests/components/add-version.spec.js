...
describe("[Functional Test] Starter login", () => {
  ...
  test.describe("When I add a new version to a component", () => {
    test("We should see the version we added to the component", async () => {
      await loginPage.login("managerqa", "managerqa");
      await loginPage.closeModalIfVisible();
      await loginPage.verifyLoginSuccess();

      await mainPage.navigateToNewComponent();

      await componentsPage.createNewComponent();

      await mainPage.navigateToComponentList();

      await componentsPage.goToComponent('QATestCompo');
      await componentsPage.verifyComponentIsShown();
      await componentsPage.addNewComponentVersion();
      await componentsPage.verifyNewComponentVersion();

      await mainPage.navigateToComponentList();

      await componentsPage.deleteComponent('QATestCompo');
    });
  });
